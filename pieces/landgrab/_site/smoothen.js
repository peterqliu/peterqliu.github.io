...
self.addEventListener('message', function(e) {
	...
	if (radius<1) return
	var data = e.data;
    var radius = data.radius * 6;
    var smoothened = data.output.elevations

    //add rows to north and south edges
    var firstRow = smoothened.slice(0, data.output.columns);
    var lastRow =  smoothened.slice(smoothened.length - data.output.columns, smoothened.length)
    
    for (var v = 0; v<radius; v++) smoothened = firstRow.concat(smoothened.concat(lastRow));    

    //add columns to west and east edges
    for (var p = smoothened.length-1; p>=0; p -= data.output.columns) {

      var eastPx = smoothened[p];
      var westPx = smoothened[p - data.output.columns+1];

      for (var r=0; r<radius; r++) smoothened.splice(p,0, eastPx)
      for (var r=0; r<radius; r++) smoothened.splice(p-data.output.columns+1, 0, westPx)

    }

    var bufferedCols = data.output.columns+2*radius;
    var bufferedRows = data.output.rows+2*radius;

    //smoothen
    var nd = ndarray(smoothened, [bufferedRows, bufferedCols]);
    var foo = blur(nd, radius/4);

    //trim the buffer back out

    var indices = [];
    var startSpot = radius*(bufferedCols+1);

    for (var r = 0; r<data.output.rows; r++){
      for (var c = 0; c<data.output.columns; c++) {
        indices.push(r*bufferedCols+c+startSpot)
      }
    }

    smoothened = indices.map(function(d){
      return smoothened[d]
    })

	self.postMessage(smoothened);
...
},{"ndarray":4,"ndarray-gaussian-filter":15}],2:[function(require,module,exports){
"use strict"
...
},{"uniq":20}],8:[function(require,module,exports){
"use strict"
...
},{"cwise-compiler":6}],10:[function(require,module,exports){
"use strict"
...
},{"./lib/fft-matrix.js":14,"ndarray":17,"ndarray-ops":16,"typedarray-pool":19}],14:[function(require,module,exports){
...
},{"bit-twiddle":5}],15:[function(require,module,exports){
"use strict"
...
},{"cwise/lib/wrapper":9,"dup":10,"ndarray":17,"ndarray-fft":13,"ndarray-ops":16,"next-pow-2":18,"typedarray-pool":19}],16:[function(require,module,exports){
"use strict"
...
},{"cwise-compiler":6}],17:[function(require,module,exports){
arguments[4][4][0].apply(exports,arguments)
},{"dup":4,"iota-array":11,"is-buffer":12}],18:[function(require,module,exports){
...
},{"bit-twiddle":5,"buffer":22,"dup":10}],20:[function(require,module,exports){
"use strict"
