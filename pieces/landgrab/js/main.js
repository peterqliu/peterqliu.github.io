...
// 	app.setState('preview', 'smoothenUI', !state.mostCommonStride == 0)
// 	d3.select('#smoothen')
...
var app = {
	...
	setState: (module, key, value) =>{
		...
		switch (key) {
			...
			case 'expanded':  
				...
				app.setState('map', 'drawing', false);
			...
			case 'smoothen':
				...
				if (!state.preview.loading) {
					app.setState('preview', 'loading', true)

					window.setTimeout(function(){
						app.renderer.makeMesh({}, app.setState('preview', 'loading', false))
					}, 10)
				};
	...
	updateCorners: (e) => {
		var pt = app.utils.lngLatArray(e.lngLat);
	...
	makeQuery: () => {
		...
		app.setState('preview', 'loading', true);
		app.setState('preview', 'smoothenUI', false);
		app.setState('preview', 'smoothen', false);
		app.data.identifyTiles(function(d){
			app.data.getElevations(d, function(e){
				app.renderer.makeMesh({reset: true}, app.setState('preview', 'loading', false))
			})
		});
	...
	data: {
		...
		identifyTiles: (cb) => {
			...
			state.zoom = Math.floor(22- app.utils.getBaseLog(2,Math.max.apply(Math, tileExtent))+0.5)
			...
			var ulTile = ul.map( px => Math.floor(px/state.tileSize));
			...
			Promise.all(toLoad.map(t => loadTile(t)))
				.then(data => Promise.all(
					data.map(d => drawTile(state.context, d.img, d.px, d.py))
				))
	...
	utils: {
		...
		getBaseLog: (base, result) => {
			return Math.log(result) / Math.log(base);
		},

		//express distances as rounded meters/kilometers
		formatNumber: (num) => {
	    ...
	    meterToPx: (m) => {
	      ...
	      return (m- state.output.min) / app.utils.mPerPixel(avgLat);
	    }
	...
	var texture = new THREE.TextureLoader()
		.load(
			...
			function(err, resp){
				...
				if (state.preview.texture === 'satellite') app.setState('preview', 'texture', 'satellite')
...
function getHypsometricImage(){
	...
	if (state.preview.texture === 'hypsometric') app.setState('preview', 'texture', 'hypsometric')
