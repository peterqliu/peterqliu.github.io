...
app.renderer = {
  ...
  init: () => {
    ...
    // THREE.JS PIECE: Setting up the renderer, scene, camera
      ...
      state.controls = new THREE.OrbitControls(camera, renderer.domElement); 
  ...
  makeMesh: (options, cb) => {
    ...
    for (var i = 0; i < geometry.vertices.length; i++) geometry.vertices[i].z = app.utils.meterToPx(elevs[i]);
    ...
    app.menu.updateDownloadPayload();

    if (options.reset) state.controls.reset();
    ...
    function buildWall(){
      ...
      var wallBottom = app.utils.meterToPx(bottom);
    ...
    function buildAxes(){
      ...
      for (var i = 0 ; i < 2; i++){
        ...
        var geometry = new THREE.TextGeometry( app.utils.formatNumber(state.dimensions[i]), {
