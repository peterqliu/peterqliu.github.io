...
function setSession(num) {
    ...
    // get list of states in session
    const statesInSession = Array.from(
        new Set(sessionData[activeSession].map(s=>s.statename))
    );

    // show labels only for those states
    map
        .setFilter('place-label',
            [
                'all',
                [
                    "in",
...
function setPortraitSprite(name, imgPath){
    ...
    map.loadImage(
        ...
        (error, image) => {
            ...
            // console.log(map.hasImage(name), name)
